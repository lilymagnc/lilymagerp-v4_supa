/**
 * 릴리맥 NC 이스트폴점 스케줄 -> 구글 캘린더 동기화 스크립트 (양식 자동맞춤형)
 * 
 * [사용 방법]
 * 1. NC 이스트폴점 스케줄 시트에서 [확장 프로그램] > [Apps Script] 클릭
...
function onOpen() {
    ...
    ui.createMenu('📅 스케줄 관리')
        .addItem('현재 시트 스케줄을 캘린더로 전송', 'syncNcSchedule')
        .addToUi();
}

function syncNcSchedule() {
    const ui = SpreadsheetApp.getUi();
    ...
    for (let r = headerRowIndex + 1; r < data.length; r++) {
        ...
        for (const block of blocks) {
            ...
            if (timeStr.includes('휴무') || timeStr.includes('연차') || timeStr.includes('휴가') || timeStr.includes('지원')) continue;

            const timeMatch = timeStr.match(/(\d+):(\d+)\s*[-~]\s*(\d+):(\d+)/);
            if (!timeMatch) {
                // 특이사항이 시간 형태가 아니면 건너뜀
                continue;
            }
            ...
            // NC 이스트폴점 영업시간(10:00~22:00 등) 에 맞춘 근무 형태 판별
            let shiftLabel = "";
            const isOpen = (sh <= 10);    // 10시쯤 출근이면 오픈
            const isClose = (eh >= 22);   // 22시쯤 퇴근이면 마감
