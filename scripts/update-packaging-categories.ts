...
async function updatePackagingCategories() {
    ...
    for (const item of allMaterials) {
        ...
        // Check if name contains '플러드' or 'opp'
        if (lowerName.includes('플러드') || lowerName.includes('opp')) {
            // Only update if not already set
            if (item.main_category !== '소모품 및 부자재' || item.mid_category !== '포장재') {
                toUpdate.push({
                    id: item.id,
                    name: item.name
                });
    ...
    for (const item of toUpdate) {
        const { error } = await supabase
            .from('materials')
            .update({ main_category: '소모품 및 부자재', mid_category: '포장재' })
            .eq('id', item.id);
...
updatePackagingCategories().catch(console.error);
