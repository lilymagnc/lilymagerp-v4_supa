...
async function updateFormflowerCategories() {
    ...
    for (const item of allMaterials) {
        ...
        if (item.name.includes('해바라기') || item.name.includes('수국') || item.name.includes('튤립')) {
            newMain = '생화';
            newMid = '폼플라워';
        }
...
updateFormflowerCategories().catch(console.error);
