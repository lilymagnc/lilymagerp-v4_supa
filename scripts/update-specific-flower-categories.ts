...
const categoryRules = [
    { keywords: ['장미'], main: '생화', mid: '장미류' },
    { keywords: ['리시안', '리시안서스'], main: '생화', mid: '리시안서스류' },
    { keywords: ['거베라'], main: '생화', mid: '거베라류' },
    { keywords: ['퐁퐁', '소국', '대국', '국화'], main: '생화', mid: '국화류' },
    { keywords: ['카네이션'], main: '생화', mid: '카네이션류' }
];

async function updateFlowerCategories() {
    ...
    for (const item of allMaterials) {
        ...
        for (const rule of categoryRules) {
            if (rule.keywords.some(kw => name.includes(kw))) {
                targetMain = rule.main;
                targetMid = rule.mid;
                break;
            }
        }

        if (targetMain && targetMid) {
    ...
    console.log(`${toUpdate.length} materials matched flower keywords and need category update.`);
    ...
    console.log('----------------------------------------------------');
    console.log(`Job Complete. Total materials categories updated: ${totalUpdated}`);
}

updateFlowerCategories().catch(console.error);
