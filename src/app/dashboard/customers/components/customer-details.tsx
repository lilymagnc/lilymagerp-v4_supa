...
export function CustomerDetails({ isOpen, onOpenChange, onEdit, customer }: CustomerDetailsProps) {
  ...
  useEffect(() => {
    if (isOpen && customer && !ordersLoading) {
      ...
      const history = orders
        ...
        .sort((a, b) => {
          const dateA = a.orderDate instanceof Date ? a.orderDate.getTime() : new Date(a.orderDate).getTime();
          const dateB = b.orderDate instanceof Date ? b.orderDate.getTime() : new Date(b.orderDate).getTime();
          return dateB - dateA;
        });
  ...
  const handleDownloadOrderHistory = () => {
    ...
    const dataToExport = orderHistory.map(order => ({
      "주문일": order.orderDate instanceof Date ? format(order.orderDate, "yyyy-MM-dd HH:mm") : (typeof order.orderDate === 'string' ? format(new Date(order.orderDate), "yyyy-MM-dd HH:mm") : '-'),
        ...
        <div className="py-4 space-y-4 max-h-[70vh] overflow-y-auto pr-4">
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-3 items-center gap-4">
              ...
              <p className="col-span-2 text-sm">{customer.weddingAnniversary ? format(new Date(customer.weddingAnniversary), "MM월 dd일", { locale: ko }) : '-'}</p>
            </div>
