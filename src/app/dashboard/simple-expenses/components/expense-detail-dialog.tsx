        ...
        <FormProvider {...form}>
          <form onSubmit={form.handleSubmit(handleSave)} className="space-y-4">
            ...
            <div className="text-xs text-gray-500 space-y-1">
              <div>생성일: {expense.createdAt ? format(new Date(expense.createdAt), 'yyyy-MM-dd HH:mm', { locale: ko }) : '-'}</div>
              {expense.updatedAt && (
                <div>수정일: {format(new Date(expense.updatedAt), 'yyyy-MM-dd HH:mm', { locale: ko })}</div>
              )}
            </div>
