...
export function OrderEditDialog({ isOpen, onOpenChange, order }: OrderEditDialogProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const originalPayment = (order.payment || {}) as any;

      let completedAt = originalPayment.completedAt;
      if ((paymentStatus === 'paid' || paymentStatus === 'completed') && !completedAt) {
      ...
      const updatedOrder = {
        ...
        actualDeliveryCostCash: actualDeliveryCostCash,
        ...
        deliveryProfit: (formData.receiptType === 'delivery_reservation') ? (calculateTotal().deliveryFee - (actualDeliveryCost + actualDeliveryCostCash)) : order.deliveryProfit
      };
