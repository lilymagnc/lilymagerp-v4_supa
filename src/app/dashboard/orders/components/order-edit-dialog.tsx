...
export function OrderEditDialog({ isOpen, onOpenChange, order }: OrderEditDialogProps) {
  ...
  const createDeliveryExpense = async (order: Order, actualCost: number, actualCostCash: number) => {
    try {
      ...
      // 지출 날짜는 배송일 기준 (formData.pickupDate 가 배송일/픽업일 임)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 배송비 정보 수정 여부 확인
      const isDeliveryCostChanged =
        actualDeliveryCost !== order.actualDeliveryCost ||
        actualDeliveryCostCash !== order.actualDeliveryCostCash ||
        formData.pickupDate !== (order.deliveryInfo?.date || order.pickupInfo?.date);

      // 실제 배송비 정보가 있고, 변경되었을 때만 지출 내역 생성/업데이트
      if (formData.receiptType === 'delivery_reservation' && isDeliveryCostChanged) {
