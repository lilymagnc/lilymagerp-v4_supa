...
export default function PickupDeliveryPage() {
  ...
  const createDeliveryExpense = async (order: Order, actualCost: number, actualCostCash: number = 0) => {
    try {
      ...
      if (!deliveryDate) {
        console.warn('작업 날짜(배송/픽업일)가 없어 지출을 생성할 수 없습니다.');
        return;
      }
      const targetDate = new Date(deliveryDate);
      if (isNaN(targetDate.getTime())) {
        console.warn('유효하지 않은 날짜 형식입니다:', deliveryDate);
        return;
      }
      ...
      if (nonCashAmount > 0) {
        const updated = await updateExpenseByOrderId(order.id, expenseData, 'DELIVERY');
        if (!updated) {
      ...
      if (actualCostCash > 0) {
        const cashExpenseData = {
          date: targetDate,
          category: SimpleExpenseCategory.TRANSPORT,
          subCategory: 'DELIVERY_CASH',
          description: `배송비-${order.deliveryInfo?.driverAffiliation || '운송업체'} 현금${actualCostCash.toLocaleString()}원`,
        ...
        const updated = await updateExpenseByOrderId(order.id, cashExpenseData, 'DELIVERY_CASH');
        if (!updated) {
          await addExpense(cashExpenseData, orderBranch.id, orderBranch.name);
        }
      } else {
        await deleteExpenseByOrderId(order.id, 'DELIVERY_CASH');
      }
    } catch (error) {
      console.error('배송비 자동 지출 생성 오류:', error);
      toast({ variant: 'destructive', title: '배송비 연동 오류', description: '지출 내역 자동 등록 중 오류가 발생했습니다.' });
    }
  };
  ...
  const handleUpdateDriverInfo = async () => {
    ...
    try {
      ...
      if (editingDriverInfo.actualDeliveryCost?.trim() || editingDriverInfo.actualDeliveryCostCash?.trim()) {
        ...
        // OrderEditDialog와 동일한 createDeliveryExpense 로직을 위해 PickupDeliveryPage의 createDeliveryExpense도 업데이트 필요
        // 하지만 여기서는 간단히 하기 위해 일단 actualCost 기준으로 지출 생성
        // 더 정확하게는 cash 부분도 생성해야 함
        await createDeliveryExpense(order, actualCost, actualCostCash);
      }
