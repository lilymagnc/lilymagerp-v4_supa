...
export default function PickupDeliveryPage() {
  ...
  }, [viewMode, startDate, endDate, dateFilterType, currentCalendarDate, fetchOrdersBySchedule, fetchOrdersByRange, activeTab]);
  ...
  const handleUpdateDriverInfo = async () => {
    ...
    try {
      ...
      if (editingDriverInfo.actualDeliveryCost?.trim() || editingDriverInfo.actualDeliveryCostCash?.trim()) {
        ...
        updateData.actualDeliveryCostCash = actualCostCash;
      ...
      toast({ variant: 'destructive', title: '오류', description: '정보 업데이트 중 오류가 발생했습니다.' });
    }
  };
  ...
  const handleSaveDeliveryCost = async () => {
    ...
    try {
      ...
      const updateData: any = {
        ...
        actualDeliveryCostCash: actualCostCash,
      ...
      // 배송 소속 정보 업데이트
      if (deliveryAffiliation || selectedOrderForCost.deliveryInfo) {
        updateData.deliveryInfo = {
          ...(selectedOrderForCost.deliveryInfo || {}),
          driverAffiliation: deliveryAffiliation || selectedOrderForCost.deliveryInfo?.driverAffiliation || ''
        };
      }
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      // 배송일/픽업일 기준으로 정렬
      const dateA = a.receiptType === 'delivery_reservation'
        ? (a.deliveryInfo?.date || '')
        : (a.pickupInfo?.date || '');
      const dateB = b.receiptType === 'delivery_reservation'
        ? (b.deliveryInfo?.date || '')
        : (b.pickupInfo?.date || '');
      ...
      const timeB = b.receiptType === 'delivery_reservation'
  ...
  const completedDeliveryOrders = useMemo(() =>
    deliveryOrders.filter(o =>
      (o.status === 'completed' || o.actualDeliveryCost !== undefined) &&
      ((o.summary?.deliveryFee || 0) > 0 || (o.actualDeliveryCost || 0) > 0 || (o.actualDeliveryCostCash || 0) > 0)
    ), [deliveryOrders]);
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid grid-cols-4 w-full h-12 bg-slate-100 p-1">
            ...
            <TabsTrigger value="costs" className="rounded-md data-[state=active]:bg-white data-[state=active]:shadow-sm">
              차액 정산 <Badge variant="secondary" className="ml-1.5 h-5 px-1 bg-slate-200 text-slate-600 border-none font-bold">{completedDeliveryOrders.filter(o => !o.actualDeliveryCost).length}</Badge>
            </TabsTrigger>
            ...
            <DeliveryTable
              ...
              onEditDriver={(order) => {
                setEditingDriverInfo({
                  ...
                  actualDeliveryCostCash: order.actualDeliveryCostCash?.toString() || '',
                });
            ...
            <div className="bg-white p-4 rounded-xl border mb-6 flex justify-between items-center shadow-sm">
              ...
              <div className="flex gap-4">
                ...
                <div className="text-right border-r pr-4">
                  ...
                  <p className="font-bold text-red-600">₩{completedDeliveryOrders.reduce((sum, o) => sum + (o.actualDeliveryCost || 0) + (o.actualDeliveryCostCash || 0), 0).toLocaleString()}</p>
                </div>
                <div className="text-right">
                  <p className="text-[10px] text-slate-400">전체 배송 수익</p>
                  <p className="font-bold text-emerald-600">₩{completedDeliveryOrders.reduce((sum, o) => sum + (o.deliveryProfit || 0), 0).toLocaleString()}</p>
                </div>
              </div>
            </div>
            <DeliveryCostTable
              ...
              onCostInput={(order) => {
                ...
                setDeliveryCost(order.actualDeliveryCost?.toString() || '');
                setDeliveryCostCash(order.actualDeliveryCostCash?.toString() || '');
                setDeliveryAffiliation(order.deliveryInfo?.driverAffiliation || '');
          ...
          <div className="grid gap-4 py-4">
            ...
            <div className="grid gap-2">
              ...
              <Input
                ...
                value={editingDriverInfo?.actualDeliveryCostCash}
                onChange={(e) => setEditingDriverInfo(prev => prev ? { ...prev, actualDeliveryCostCash: e.target.value } : null)}
